        ...
        for pdb_file in pdb_files:
            if plot_visibility[pdb_file]:
                if core == "pepsisaxs" or core == "both":
                    ...
                    line, = ax.plot(saxs_output.q, saxs_output.int, linewidth=2, linestyle='--')
                    handles.append(line)
...
def fitsans(pdb_files, data_file, deut_level=[0], d2o_level=[0], exchange=[0], conc=1, abs_int=0, hyd=False, bulk_SLD=1e-5, no_smear=False, scale=1, bg=0, neg=False, hyd_shell=5):
    if isinstance(pdb_files, str):
        pdb_files = [pdb_files]
    fit_file = []
    values_fit =[]
